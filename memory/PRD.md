...
### Hybrid Adaptive Question Engine (NEW - Mar 2026)
- **Stage 1 — Core Deterministic Questions**: 5 fixed questions (core_1 to core_5) served in order, feeding the scoring model
- **Stage 2 — AI-Generated Follow-ups**: Up to 3 contextual follow-up questions generated by Claude Sonnet 4.5
...
- **Constraints**: Follow-ups only triggered when strong signals detected; max 3 follow-ups; LLM generates question text only, does not influence scoring
- **UX**: Clear stage transition with amber "Adaptive Investigation" banner, AI FOLLOW-UP X/3 badge
- **Fallback**: Deterministic fallback questions if LLM unavailable
...
## Prioritized Backlog
### P2 - Upcoming
- [ ] "Mirror Mode": Answer questions from partner's perspective
- [ ] "Conversation Coach": AI-guided tool for preparing difficult conversations
### P2 - Future
